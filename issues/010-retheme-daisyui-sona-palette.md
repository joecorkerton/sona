...
## Acceptance criteria
- [ ] `assets/css/app.css` keeps `@plugin "daisyui/packages/bundle/daisyui"` (`themes: false`)
- [ ] `assets/css/app.css` replaces the two stock `@plugin ".../daisyui-theme"` blocks (`dark`
      and `light`) with a single custom `sona` theme: `name: "sona"`, `default: true`,
      `prefersdark: false`, `color-scheme: "light"`
- [ ] The `sona` theme maps daisyUI semantic tokens to the Sona palette: `--color-base-100`
      ...
      generator)
- [ ] `assets/css/app.css` removes the `dark` `@custom-variant`
      (`@custom-variant dark (&:where([data-theme=dark], [data-theme=dark] *))`) — single light
      theme only
- [ ] `assets/css/app.css` adds a small `@theme` block exposing raw Sona utilities
...
- [ ] `@import "tailwindcss" source(none);`, the heroicons `@plugin`, the colocated `@source`
...
- [ ] No `@apply` used in `app.css`
- [ ] `mix.exs` still lists `{:daisyui, ...}` (v5); no dependency changes
- [ ] Topbar progress bar color set to Sona teal (`#0d9488`) in `assets/js/app.js` (was `#29d`)
- [ ] No new Elixir dependencies added
