...
## Acceptance criteria
- [x] `lib/sona_web/router.ex` adds `live "/guide", GuideLive` inside the existing `live_session :current_user` (no new live_session, current_scope flows through)
- [x] Template starts with `<Layouts.app flash={@flash} current_scope={@current_scope}>`
- [x] `mount/3` auto-ensures the conversation via `Sona.Guide.ensure_conversation/1` — opening `/guide` with no existing conversation renders the `hidden only:block` empty state (no "Set up" gate); the conversation row exists before any message is sent
- [x] Messages render from a `:guide_messages` **stream** (not a list) with `phx-update="stream"` + a DOM id on the parent; each child keyed by its streamed id
- [x] The guide's messages are visually distinct from a coworker: header shows "Sona Guide" + a guide icon; guide bubbles use a guide accent (left-aligned assistant, right-aligned user), consistent with `RoomLive` but clearly the LLM
- [x] Composer form has `id="guide-compose-form"` and `phx-submit="send"`; it is sticky and usable at ~390px (mobile-first)
- [x] On `send`, `Sona.Guide.send_user_message/2` is called and the user message is **not** inserted locally — the broadcast round-trip drives the insert (single insert per client)
- [x] `handle_info({:new_guide_message, msg}, socket)` does `stream_insert(:guide_messages, msg, at: -1)`; the composer re-enables only when `msg.role == :assistant`; a `:user` broadcast keeps "thinking" on
- [x] Error path: on `{:error, _}` from `send_user_message/2` a flash error is shown and the user's text stays in the input (retryable)
- [x] Re-opening `/guide` restores the full guide history (proactive + exchanges)
- [x] `test/sona_web/live/guide_live_test.exs` asserts: open `/guide`, see seeded proactive guide bubble, submit a follow-up via `render_submit/2` on `#guide-compose-form`, see the stub assistant reply as a new guide bubble, single insert per client, composer disabled state handled, and the error path (flash + retained input) — all network-free via `Sona.Guide.LLM.Stub`
...
- 2026-07-12: started implementation — adding `live "/guide", GuideLive` to the router, writing `SonaWeb.GuideLive` + template (modeled on `RoomLive`), and the test file. Stub LLM is already configured for the test env (per 015).
- 2026-07-12: landed `lib/sona_web/router.ex` (added `live "/guide", GuideLive` inside the existing `live_session :current_user`); `lib/sona_web/live/guide_live.ex` (mount auto-ensures + subscribe-on-connect + send handler that calls `Sona.Guide.send_user_message/2` with no local insert + handle_info that `stream_insert`s and re-enables the composer only on `:assistant`) and `lib/sona_web/live/guide_live.html.heex` (sticky guide header with sparkles + LLM-themed bubbles via `bg-sona-lime` for assistant vs `bg-primary` for user + sticky composer with `disabled={@thinking}` + "thinking" pill while waiting)
- 2026-07-12: `test/sona_web/live/guide_live_test.exs` (16 tests) — all pass via `Sona.Guide.LLM.Stub`, no network; covers mount+empty state, header+composer attrs, send→user+assistant bubbles with single-insert assertion, proactive+follow-up history, error path (flash + retained text + re-enabled composer), history restoration on re-open, and visual alignment (assistant left, user right). Full `mix precommit` clean (181 tests, credo --strict clean).
- 2026-07-12: completed — all acceptance criteria met; mix precommit clean (181 tests, 0 credo issues).
- 2026-07-12: post-review fixes — (1) refactored `test/sona_web/live/guide_live_test.exs` to drop all raw-HTML regex assertions; disabled state, empty-state visibility, visual alignment, and single-insert are now asserted via `has_element?` with CSS selectors and a small `count_message_bubbles/2` helper using `LazyHTML.query("[data-role]")` over rendered HTML (no regex over the document). (2) `issues/TODO.md` rebuilt: 020 and 021 are `status: todo` in their frontmatter and correctly sit under `## Todo` (they were never in-progress in this slice).