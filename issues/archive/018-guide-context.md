...
- `latest_guide_summary(%User{})` — returns `%{teaser: String.t() | nil, inserted_at: NaiveDateTime.t()}` (the map, **not** the atom `nil`) for the Inbox Guide section. **Auto-ensures** the conversation (same idempotent `ensure_conversation/1`), so for a signed-in user it always returns the map; the `teaser` field is `nil` only when the conversation has no messages yet (fresh, unseeded user).
...
## Acceptance criteria
- [x] `lib/sona/guide.ex` exposes `ensure_conversation/1`, `list_messages/1`, `latest_guide_summary/1`, `send_user_message/2`, `seed_proactive_message/2`, `subscribe_guide/1`; every lookup is company-scoped via `user.company_id` (`list_messages/1` accepts either `%User{}` or `%GuideConversation{}`, but a `%User{}` is scoped to that user's conversation and a `%GuideConversation{}` is verified to belong to the caller's company); LiveViews call only this module (never `Repo`)
- [x] `ensure_conversation/1` is idempotent: concurrent calls yield one conversation row (rescues the `user_id` unique constraint and re-fetches)
- [x] `latest_guide_summary/1` auto-ensures the conversation and always returns the summary **map** for a signed-in user (never the atom `nil`); the map's `teaser` is `nil` only when no messages exist yet (fresh, unseeded user)
- [x] `send_user_message/2` inserts a `:user` message, broadcasts `{:new_guide_message, msg}`, rebuilds the system prompt via `Sona.Guide.Prompt.build/2` + `Sona.Guide.ShiftData.for/1`, calls `Sona.Guide.LLM.impl().reply/3`, inserts an `:assistant` message, and broadcasts `{:new_guide_message, assistant_msg}`; returns `{:ok, assistant_msg}` on success
- [x] `history` passed to `reply/3` is the full list of prior `guide_messages` oldest→newest mapped to `%{role, content}`, **including** the seeded proactive `:assistant` message (not filtered out)
- [x] `send_user_message/2` error path (stub returns `{:error, _}`) returns `{:error, _}`, and the chosen `:user`-message-on-error behaviour (insert-then-return-error, or rollback/no-insert) is **recorded in `## Notes`** with a one-line rationale before this criterion is checked off
- [x] `seed_proactive_message/2` inserts an `:assistant` message, makes **no** LLM call, invokes **no** `Prompt.build/2`, and re-fetches the conversation internally (caller passes only `%User{}` + body)
- [x] Company scoping: a user from company B cannot read or act on company A's guide conversation; all lookups filter by `user.company_id`
- [x] PubSub topic is `"guide:user:#{user_id}"`; both broadcasts use the `{:new_guide_message, msg}` tag (no separate assistant event)
- [x] `Sona.Guide` does **not** call `Sona.Chats` (and vice versa)
- [x] `test/sona/guide_test.exs` covers all the above using `Sona.Guide.LLM.Stub` (no network, no `ANTHROPIC_API_KEY`); tests pass
...
- 2026-07-12: started implementation — creating `lib/sona/guide.ex` and `test/sona/guide_test.exs`
- 2026-07-12: `lib/sona/guide.ex` written with all six public functions — context module compiles clean
- 2026-07-12: all 19 tests pass — `test/sona/guide_test.exs` covers all criteria
- 2026-07-12: **Error-path decision**: user message is kept (not rolled back) when the LLM call fails. Rationale: the user can see what they sent even if the AI fails to respond, and the broadcast of the user message has already fired. Rolling back would require a transaction around the LLM call, which is impractical since the broadcast also needs to happen. The `:user`-insert + broadcast + LLM-fail + `{:error, reason}` sequence is the chosen behaviour.
- 2026-07-12: completed — all acceptance criteria met; mix precommit clean (165 tests)
- 2026-07-12: post-review fixes — narrowed `create_conversation/1` rescue to only catch the `:user_id` unique constraint (was over-broad); fixed `latest_guide_summary/1` spec/doc from `DateTime.t()` to `NaiveDateTime.t()` to match schema defaults; documented the `%GuideConversation{}` trust model in `list_messages/1` docs (the 1-arg form cannot verify company scoping — the caller is trusted since they obtained the conversation via `ensure_conversation/1`)